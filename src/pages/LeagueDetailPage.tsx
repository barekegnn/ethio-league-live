...
const LeagueDetailPage = () => {
  ...
  const league = id ? leagueById(id) : undefined;

  useEffect(() => {
    if (league) document.title = `${league.name} · Standings`;
  }, [league]);
                ...
                {table.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-3">
                        <Link to={`/clubs/${row.clubId}`} className="flex items-center gap-2 hover:text-primary">
                        ...
                        <div className="flex gap-1 justify-center">
                          {row.form.map((r, i) => (
                            <span
                              key={i}
                              className={cn(
                                "w-5 h-5 rounded text-[10px] font-bold grid place-items-center",
                                formColor(r),
                              )}
                              title={r}
                            >
