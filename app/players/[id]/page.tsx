...
export default function PlayerDetailPage() {
  ...
  const player = params.id ? playerById(params.id) : undefined;
      ...
      >
        <div className="mx-auto max-w-4xl px-3 sm:px-6 py-6 sm:py-10 flex items-center gap-4">
          ...
          <div className="min-w-0">
            <div className="text-xs uppercase tracking-wider opacity-80">#{player.number} · {player.position}</div>
            <h1 className="font-display text-2xl sm:text-4xl font-bold tracking-tight">{player.name}</h1>
            {club && (
              <Link href={`/clubs/${club.id}`} className="inline-flex items-center gap-2 mt-2 text-sm text-white/90 hover:text-white">
      ...
      <div className="mx-auto max-w-4xl px-3 sm:px-6 py-4 sm:py-6 space-y-5">
        ...
        <div className="bg-card rounded-xl border border-border p-4 shadow-[var(--shadow-card)]">
          ...
          <dl className="grid grid-cols-2 gap-y-1 text-sm">
            <dt className="text-muted-foreground">Nationality</dt><dd>{player.nationality}</dd>
            <dt className="text-muted-foreground">Date of birth</dt><dd>{new Date(player.dateOfBirth).toLocaleDateString()}</dd>
            <dt className="text-muted-foreground">Age</dt><dd>{age}</dd>
            <dt className="text-muted-foreground">Position</dt><dd>{player.position}</dd>
          </dl>
        </div>
      </div>
    </div>
