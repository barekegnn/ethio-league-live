...
export default function MatchDetailPage() {
  ...
  const match = params.id ? matchById(params.id) : undefined;
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-12 text-center">
        ...
        <Button asChild variant="ghost" className="mt-4">
          <Link href="/matches"><ArrowLeft className="w-4 h-4" /> Back to matches</Link>
        </Button>
      </div>
    );
  }
        ...
        <div className="px-3 sm:px-6 pt-3 pb-6 sm:pb-8">
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 sm:gap-8">
            <Link href={`/clubs/${match.homeId}`} className="flex flex-col items-center gap-2">
              ...
              <span className="font-display font-bold text-base sm:text-xl text-center">{home?.shortName}</span>
            </Link>
            ...
            <Link href={`/clubs/${match.awayId}`} className="flex flex-col items-center gap-2">
              ...
              <span className="font-display font-bold text-base sm:text-xl text-center">{away?.shortName}</span>
            </Link>
          </div>
          ...
          <TabsContent value="summary" className="mt-4 space-y-3">
            <div className="bg-card rounded-xl border border-border p-4 shadow-[var(--shadow-card)]">
              ...
              <dl className="grid grid-cols-2 gap-y-1 text-sm">
                <dt className="text-muted-foreground">Competition</dt><dd>{league?.name}</dd>
                <dt className="text-muted-foreground">Matchday</dt><dd>{match.matchday}</dd>
                <dt className="text-muted-foreground">Stadium</dt><dd>{match.stadium}</dd>
                <dt className="text-muted-foreground">Kickoff</dt><dd>{new Date(match.kickoff).toLocaleString()}</dd>
                {match.attendance && (<><dt className="text-muted-foreground">Attendance</dt><dd>{match.attendance.toLocaleString()}</dd></>)}
              </dl>
            </div>
          </TabsContent>
                  ...
                  return (
                    <div key={i} className={cn("p-3 flex items-center gap-3", isHome ? "" : "flex-row-reverse text-right")}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground capitalize">{e.type === "goal" ? "Goal" : e.type}</div>
                      </div>
