...
import { clubById, leagueById, matchesByLeague, playersByClub, standings } from "@/data/mock";
...
export default function ClubDetailPage() {
  ...
  const club = params.id ? clubById(params.id) : undefined;
  ...
  const fixtures = allMatches.filter((m) => m.status !== "completed").sort((a, b) => +new Date(a.kickoff) - +new Date(b.kickoff));
        ...
        <div className="mx-auto max-w-5xl px-3 sm:px-6 py-6 sm:py-10 flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs sm:text-sm text-white/80">
              ...
              {standing && <span className="inline-flex items-center gap-1 font-semibold">· #{standing.position} · {standing.points} pts</span>}
            </div>
          </div>
        </div>
      </section>
        ...
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="squad" className="mt-4">
            <div className="bg-card rounded-xl border border-border shadow-[var(--shadow-card)] divide-y divide-border">
              {squad.length === 0 && (
                <div className="p-6 text-center text-sm text-muted-foreground">Squad data coming soon.</div>
              )}
              {squad.map((p) => (
